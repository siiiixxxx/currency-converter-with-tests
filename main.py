...
class CurrencyConverterApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        tk.Label(self, text="Сумма кредита (₽):").grid(
            row=0, column=0, sticky="w", **pad
        )
        self.loan_var = tk.StringVar()
        tk.Entry(self, textvariable=self.loan_var).grid(
            row=0, column=1, sticky="ew", **pad
        )

        tk.Label(self, text="Срок кредита (мес):").grid(
            row=1, column=0, sticky="w", **pad
        )
        self.time_var = tk.StringVar()
        tk.Entry(self, textvariable=self.time_var).grid(
            row=1, column=1, sticky="ew", **pad
        )

        tk.Label(self, text="Годовая ставка (%):").grid(
            row=2, column=0, sticky="w", **pad
        )
        self.interest_var = tk.StringVar()
        tk.Entry(self, textvariable=self.interest_var).grid(
            row=2, column=1, sticky="ew", **pad
        )

        tk.Button(self, text="Рассчитать кредит", command=self.calculate_loan).grid(
            row=3, column=0, columnspan=2, pady=15
        )

        tk.Label(self, text="Ежемесячный платёж (₽):").grid(
            row=4, column=0, sticky="w", **pad
        )
        self.monthly_label = tk.Label(self, text="", font=("Arial", 12, "bold"))
        self.monthly_label.grid(row=4, column=1, sticky="w", **pad)

        tk.Label(self, text="Сумма к выплате (₽):").grid(
        ...
        tk.Label(self, text="Конвертировать в:").grid(
            row=7, column=0, sticky="w", **pad
        )
        currencies = ["USD", "EUR", "CNY", "GBP"]
        ttk.Combobox(
            self, textvariable=self.target_var, values=currencies, state="readonly"
        ...
        self.result_label = tk.Label(
            self, text="", font=("Arial", 14, "bold"), fg="green"
        )
        self.result_label.grid(row=9, column=0, columnspan=2, pady=10)

        tk.Button(self, text="Обновить курсы", command=self.update_db).grid(
    ...
    def convert(self):
        try:
            monthly_rub = float(
                self.monthly_label.cget("text").replace(" ₽", "").replace(",", "")
            )
    ...
    def update_db(self):
        ...
        self.log(
            f"Курсы обновлены: {', '.join([f'{k}: {v}' for k, v in self.rates.items()])}"
        )
